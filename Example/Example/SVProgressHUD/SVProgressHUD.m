...
+ (void)setDefaultDirection:(SVProgressHUDLayoutDirection)direction {
    [self sharedView].defaultLayoutDirection = direction;
}
...
- (void)initializeDefaultConfig {
    ...
    _infoImage = [UIImage imageWithContentsOfFile:[imageBundle pathForResource:@"toast_icon_info" ofType:@"png"]];
    _successImage = [UIImage imageWithContentsOfFile:[imageBundle pathForResource:@"toast_icon_success" ofType:@"png"]];
    _errorImage = [UIImage imageWithContentsOfFile:[imageBundle pathForResource:@"toast_icon_error" ofType:@"png"]];
...
- (void)updateHUDFrame {
    ...
    if (self.defaultLayoutDirection == SVProgressHUDLayoutDirectionVertical) {
         ...
         *   ———
         */
        hudHeight = SVProgressHUDVerticalSpacing + MAX(labelHeight, contentHeight) + SVProgressHUDVerticalSpacing;
        
        /** hudWidth
         * |-spacing-content-(labelSpacing-labelWidth-)spacing-|
         */
        hudWidth = SVProgressHUDHorizontalSpacing + contentWidth + 0 + labelWidth + SVProgressHUDHorizontalSpacing;
        if(self.statusLabel.text && (imageUsed || progressUsed)){
            // Add spacing if both content and label are used
            hudWidth += SVProgressHUDLabelSpacing;
        }
    ...
    if (self.defaultLayoutDirection == SVProgressHUDLayoutDirectionVertical) {
        ...
        if(self.statusLabel.text) {
            ...
            centerX = xOffset + contentWidth / 2.0f;
        } else {
...
+ (NSTimeInterval)displayDurationForString:(NSString*)string {
    CGFloat minimum = MAX((CGFloat)string.length * 0.03 + 0.5, [self sharedView].minimumDismissTimeInterval);
...
- (UILabel*)statusLabel {
    if(!_statusLabel) {
        ...
        _statusLabel.textAlignment = NSTextAlignmentJustified;
...
- (void)setDefaultLayoutDirection:(SVProgressHUDLayoutDirection)defaultLayoutDirection {
    if (!_isInitializing) {
        _defaultLayoutDirection = defaultLayoutDirection;
    }
}
